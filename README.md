...
## Dataset
The Language Augmented TaskGrasp (LA-TaskGrasp) dataset is developed based on [TaskGrasp](https://arxiv.org/abs/2011.06431) dataset. To perform training and evaluation on the LA-TaskGrasp dataset, download the dataset [here](https://1drv.ms/u/c/3aabf3f0ed719c78/IQD1z6UwUUwrQJlft7QapoiiAZM06OAZQBuBW2OQeyRWUws) and place it in the root folder as `data`:
```shell
...
To run any of the demo scripts below, download the pre-trained models [here](https://1drv.ms/u/c/3aabf3f0ed719c78/IQALqzKZCi2WRYXVyR8EuIzvAVAFvncEAJgTKw0bILibJ7I) and put them in the `checkpoints` folder.
...
@article{tang2025foundationgrasp,
  ...
  ```
## Data
Step 1. Download the original LA-TaskGrasp dataset [here](https://1drv.ms/u/c/3aabf3f0ed719c78/IQD1z6UwUUwrQJlft7QapoiiAZM06OAZQBuBW2OQeyRWUws) and place it in the root folder as `data`.   
Step 2. Download the updated language description data of the LaViA-TaskGrasp dataset [here](https://1drv.ms/u/c/3aabf3f0ed719c78/IQCMOmHCJc0GQKI_ZNDNGeYHAYPfiFj5HhV24KTYFWBNvjs) and replace the old one in the LA-TaskGrasp dataset (i.e., obj_gpt_vx, task_gpt_vx).   
