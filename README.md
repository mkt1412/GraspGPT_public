...
## Dataset
The Language Augmented TaskGrasp (LA-TaskGrasp) dataset is developed based on [TaskGrasp](https://arxiv.org/abs/2011.06431) dataset. To perform training and evaluation on the LA-TaskGrasp dataset, download the dataset [here](https://gatech.box.com/s/3q9qjwg61dqm5qu461nqfvboe8z57an2) and place it in the root folder as `data`:
```shell
...
To run any of the demo scripts below, download the pre-trained models [here](https://gatech.box.com/s/omthr8zs1f98k4p1v3u8bhy7yxzeiudj) and put them in the `checkpoints` folder.
...
Step 2. Download the updated language description data of the LaViA-TaskGrasp dataset [here](https://gatech.box.com/s/5onzr6sj99jpcsee1sgo7gztpqui4z2p) and replace the old one in the LA-TaskGrasp dataset (i.e., obj_gpt_vx, task_gpt_vx).   
